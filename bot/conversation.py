...
from bot.keyboards import districts_keyboard, start_keyboard, FIND_GROUP_CALLBACK, types_keyboard, times_keyboard, \
    add_to_group_keyboard, cancel_keyboard
...
def conversation_handler():
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(conversation_start, FIND_GROUP_CALLBACK)
        ],
        states={
        ...
        fallbacks=[CallbackQueryHandler(conversation_cancel, 'cancel')], allow_reentry=True
    )
...
async def conversation_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        if len(times) == 1:
            selected_groups = await fetch_groups_by_params(district_callback, type_callback, None)
