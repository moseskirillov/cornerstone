...
start_keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(
                text="Подобрать группу", callback_data=FIND_GROUP_CALLBACK
            )
        ]
    ],
)
...
async def times_keyboard(district_callback, type_callback):
    times = await fetch_available_times(
        district_callback=district_callback, type_callback=type_callback
    )
